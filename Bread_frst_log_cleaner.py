...
# Remove any lines with these whitelisted strings in them
clean_strings = [
    ...
    "CHR Extension: (McAfee(R) WebAdvisor)",
    "OPR Extension: (opera-intro)",
    ...
    # "AppData\\Local\\zenmap.exe.log",
    # "AppData\\Roaming\\jd-gui.cfg",
    ...
    "F.lux Software LLC",
    "f.lux Software LLC",
    r": %windir%\system32\compattelrunner.exe",
    ...
    "Ubisoft",
    r"Chrome\\User Data\System Profile [",
...
# Always keep lines with these strings in them
# Unless it's from `Microsoft Corporation`
important_strings = [
    "<==== ATTENTION",
    "No File",
    "File not signed",
...
# Define a function to check if a line should be removed
def should_remove_line(line):
    # Check if it contains any of the clean strings and none of the important strings
    if any(clean_string in line for clean_string in clean_strings) and not any(
        important_string in line for important_string in important_strings
    ):
        return True
        # Check if it contains any of the important strings and any of the white list strings
    if any(important_string in line for important_string in important_strings):
        return False
    return False

# Clean the FRST logs
def clean_file(file_path):
    output_path = f"{file_path.replace('.txt', '')}_cleaned.txt"
    
    with open(file_path, encoding="utf-8") as oldfile, open(output_path, "w", encoding="utf-8") as newfile:
        for line in oldfile:
            line = line.replace("\u00AE", "(R)").replace("Â", "")
            if not should_remove_line(line):
                newfile.write(line)
    
    return output_path

# This will determine the listed DIR based on the OS
def get_common_dirs():
    if platform.system() == "Windows":
        ...
        # Linux directories (for home directories)
        return [
...
# This will find FRST.txt and Addition.txt in the specified DIR
def find_frst_logs():
    common_dirs = get_common_dirs()
    
    for directory in common_dirs:
        for file_name in ["FRST.txt", "Addition.txt"]:
            file_path = os.path.join(directory, file_name)
            if os.path.exists(file_path):
                print(f"Bread Found FRST logs in {directory}")
                return file_path, directory  # Return the path and directory where the file was found

    print("Bread didn't find logs in the specified directories.")
    return None, None

# Start the cleaning process
def main():
    user_input = input("Type 'clean' to clean the FRST logs: ").strip().lower()
    
    if user_input == "clean":
        log_path, log_dir = find_frst_logs()
        
        if log_path:
            # Clean both FRST.txt and Addition.txt
            cleaned_log = clean_file(log_path)
            print(f"Cleaned file saved as {cleaned_log}")
            
            # Clean Addition.txt as well if it exists in the same directory
            addition_path = os.path.join(log_dir, "Addition.txt")
            if os.path.exists(addition_path):
                cleaned_addition = clean_file(addition_path)
                print(f"Cleaned Addition file saved as {cleaned_addition}")
        else:
            print("Make sure the FRST logs exist. (っ- ‸ - ς)")
    else:
        print("Invalid input. Type 'clean' to start cleaning.")
